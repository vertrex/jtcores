...
def pack_plan(plans, size=8, flip_x=None):
    ...
    for x in range(0, size):
        for y in range(0, size):
            bin_string = (
                f"{plans[0][x][y]}{plans[1][x][y]}{plans[2][x][y]}{plans[3][x][y]}"
            )
            if flip_x:
...
def split_cpu_ram(file_name, file_path):
    f = open(f"{file_path}/{file_name}", 'rb')
    cpu_ram = f.read()
    f.close()
    sprite_ram = cpu_ram[0xd800:0xdfff]
    palette_ram = cpu_ram[0xe000:0xe7ff]
    bg1_ram = cpu_ram[0xe800:0xefff]
    bg2_ram = cpu_ram[0xf000:0xf7ff]
    vram_ram = cpu_ram[0xf800:0xfff]
    return (bg1_ram, bg2_ram, palette_ram, sprite_ram, vram_ram)
