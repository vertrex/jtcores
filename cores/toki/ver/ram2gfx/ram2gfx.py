...
# palette is divide in 4 for each zone bg1,
...
def put_pixel(img, palette,  x_pos, y_pos, color, char, palette_offset, x_size=8, y_size=8, rom_index=None):
   for y in range(0, y_size):
     for x in range(0, x_size):
       try:
         # if rom_index== 0xfbb:
           # print(f'put pixel at {x_pos +x} {y_pos+y} {hex(char[(y*y_size)+x])}')
         palette_index = ((color <<4) ^ char[(y*y_size)+x])
         converted_color = palette_to_rgb(palette, palette_index, palette_offset) 
         if converted_color != (0x00, 0x00, 0x00): #don't write full black pixel 
...
def scan_ram(img, ram, gfx_rom, palette, palette_offset, tile_size):
  for screen_index in range(int(len(ram)/2)):
    ...
    if tile_info != b'\x00\x00':
      col = int(screen_index % 32) #32*8 => 256  (32,32) 1024 tile per screen 
      row = int(screen_index / 32) #32*8 => 256
      x_pos = col*tile_size #8 or 16
      y_pos = row*tile_size #8 or 16
      rom_index = ((tile_info[0]&0xf) << 8) + tile_info[1] #4096 tile in rom
      color = (tile_info[0]  >> 4)
      ...
      put_pixel(img, palette, x_pos, y_pos, color, packed, palette_offset, x_size=tile_size, y_size=tile_size, rom_index=rom_index) 
...
def tile_bitplanes(gfx_rom, rom_index):
  ...
  if rom_index == 0xfbb:
    print(f"{(plans[2], plans[3], plans[0], plans[1],)}".replace(",","\n"))
...
def scan_sprite(img, ram, gfx_rom, palette, palette_offset, tile_size=16):
  ...
  for sprite_index in range(int(len(ram)/8)-1, -1, -1): 
    ...
    # f000 0000 f000 0000     entry not yet usedi
    ...
    put_pixel(img, palette, x, y, color, packed, palette_offset, x_size=16, y_size=16, rom_index=rom_index)
...
def ram2gfx(path, show = False):
  ...
  #scroll ?
  #mix_color(vram, bg1, bg2, sprite) #no need for the moment
...
if __name__ == "__main__":
  ...
  #rom.bin must be in current dir
  #it contain all concatened mame rom
